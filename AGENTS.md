...
- **yt-dlp Auto-Manager:** Binary is auto-downloaded from GitHub on first run and stored in the data directory. The bot self-updates daily via `yt-dlp -U`. No manual installation needed.
...
- [x] Implement `yt-dlp` stream extraction layer (with auto-download and daily update).
